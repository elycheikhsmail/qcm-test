...
test.describe("Parent — tableau de bord lecture seule", () => {
  test("/parent : moyenne globale et nb tests visibles", async ({ page }) => {
    ...
    await expect(page.locator("text=/\\d+ tests?|Aucun test/i").first()).toBeVisible({ timeout: 8_000 });
  });

  test("/parent : mention 'lecture seule' visible", async ({ page }) => {
    await page.goto("/parent");
    await page.waitForLoadState("networkidle");
    await expect(page.locator("text=lecture seule")).toBeVisible({ timeout: 8_000 });
  });
  ...
  test("/parent : indicateurs couleur (vert/amber/rouge) présents si données", async ({ page }) => {
    ...
    const indicators = page.locator(".bg-emerald-50, .bg-amber-50, .bg-red-50, text=Aucune donnée, text=—");
